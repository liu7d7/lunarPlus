...
import me.ethius.lunarplus.modifications.render.Animation;
import me.ethius.lunarplus.modifications.render.ViewModel;
...
public class ItemRenderer
{
    ...
    private void func_178104_a(AbstractClientPlayer clientPlayer, float p_178104_2_)
    {
        ...
        GlStateManager.translate(f3 * 0.70F, f3 * -0.4F, f3 * 0.0F);
        GlStateManager.rotate(f3 * 90.0F, 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(f3 * 4F, 1.0F, 0.0F, 0.0F);
        GlStateManager.rotate(f3 * 23F, 0.0F, 0.0F, 1.0F);
    }
    ...
    private void transformFirstPersonItem(float equipProgress, float swingProgress)
    {
        ...
        GlStateManager.rotate(f * (mc.thePlayer.isBlocking() ? -10F : -20.0F), 0.0F, 1.0F, 0.0F);
        GlStateManager.rotate(f1 * (mc.thePlayer.isBlocking() ? -10F : -20.0F), 0.0F, 0.0F, 1.0F);
        GlStateManager.rotate(f1 * (mc.thePlayer.isBlocking() ? -40F : -80.0F), 1.0F, 0.0F, 0.0F);
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        if (ModManager.getMod(ViewModel.class).toggled) {
            ViewModel viewModel = ((ViewModel) ModManager.getMod(ViewModel.class));
            GlStateManager.translate(viewModel.PosX.getValDouble(), viewModel.PosY.getValDouble(), viewModel.PosZ.getValDouble());
            GlStateManager.scale(viewModel.ScaleX.getValDouble(), viewModel.ScaleY.getValDouble(), viewModel.ScaleZ.getValDouble());
        }
        ...
        if (this.itemToRender != null)
        {
            ...
            else if (abstractclientplayer.getItemInUseCount() > 0)
            {
                ...
                switch (enumaction)
                {
                    ...
                    case DRINK:
                        ...
                        this.transformFirstPersonItem(f, mc.thePlayer.getSwingProgress(partialTicks));
                        break;

                    case BLOCK:
                        float var14 = MathHelper.sin(MathHelper.sqrt_float(f1) * (float)Math.PI);
                        float var15 = MathHelper.sin(f1 * f1 * 3.1415927F);
                        switch (Animation.block.getValString()) {
                            case "vanilla":
                                this.transformFirstPersonItem(f, mc.thePlayer.getSwingProgress(partialTicks));
                                this.func_178103_d();
                                break;
                            case "slide":
                                this.sigma2(f*0.3f, f1);
                                this.func_178103_d();
                                break;
                            case "sigma":
            ...
            {
                if (isCloseEnough() && mc.thePlayer.inventory.getCurrentItem() != null && mc.thePlayer.inventory.getCurrentItem().getItem() instanceof ItemSword) {
                    float var14 = MathHelper.sin(MathHelper.sqrt_float(f1) * (float)Math.PI);
                    float var15 = MathHelper.sin(f1 * f1 * 3.1415927F);
                    switch (Animation.block.getValString()) {
                        case "vanilla":
    ...
    private boolean isCloseEnough() {
        for (Entity entity : mc.theWorld.loadedEntityList) {
            if (mc.thePlayer.getDistanceToEntity(entity) < 5 && entity != mc.thePlayer && !(entity instanceof EntityItem) && !(entity instanceof EntityItemFrame) && !(entity instanceof EntityXPOrb)) {
                return true;
            }
        }
        return false;
